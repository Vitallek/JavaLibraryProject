...
const searchByKey = (e) => {
  ...
  //     setSearchKeys(res.data)
  //     setFields(Object.keys(res.data).map(field => field.charAt(0).toUpperCase() + field.slice(1)))
  //   })
  //   .catch(err => alert('can`t get data'))
}
const searchByClick = (query) => {
  alert(query)
  // axios.get(`http://${process.env.REACT_APP_SERVER_ADDR}/get-search-keys`)
  //   .then(res => {
  //     delete res.data.code
        ...
        <Stack spacing={2} sx={{ mt: 5 }} direction='column' display='flex' alignItems='center'>
          <Stack direction='row' spacing={2} display='flex' justifyContent='center' alignItems='center'>
            ...
            {searchKeys.books && <Autocomplete
              ...
              sx={{ width: 300 }}
              renderInput={(params) => <TextField {...params} label={selectedField} onKeyPress={searchByKey} />}
            />}
            <Tooltip title='Press enter' placement='right'>
              <SearchRoundedIcon sx={{ color: 'action.active', mr: 1, my: 0.5, fontSize: 30, cursor: 'pointer' }} />
            </Tooltip>
          </Stack>
          {displayedContent.length > 0 && <Stack direction="row" sx={{ maxHeight: '80vh', overflowY: 'scroll', p: 1, justifyContent: 'center', flexWrap: 'wrap' }}>
            {displayedContent.map((book, bookIndex) =>
              <BookItemCard
                key={bookIndex}
                book={book}
                bookIndex={bookIndex}
